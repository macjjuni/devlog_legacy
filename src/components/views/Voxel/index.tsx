...
const Voxel = ({ load, onLoad }: IVoxel) => {
  ...
  return (
    <div className={`${isTransparent ? 'z-[-1]' : 'z-[1]'} absolute top-0 left-0 md:h-main h-[400px] md:w-[calc(50%-32px)] w-[calc(100%-32px)]`}>
      <div className="sticky  md:top-voxelTop top-0 left-0 md:w-[calc((100vw-32px)/2)] w-full lg:w-voxelPC md:h-voxelPC h-[400px]">
        {!load && <Spinner />}
        <Canvas className={`${load ? 'opacity-100	' : ''} ${isTransparent ? 'transparent' : ''} ease duration-[0.1s]`}>
